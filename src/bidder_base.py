...
def get_colored_text(text: str, color: str = 'green') -> str:
    '''Small local replacement for langchain.input.get_colored_text (module removed in newer langchain).'''
    code = _ANSI_COLORS.get(color, '32')
    return f"\033[{code}m{text}\033[0m"


class GeminiChat:
    '''
    Minimal wrapper that talks to the Gemini REST API directly with plain
    HTTP (no google SDK). This deliberately avoids both google-generativeai
    (deprecated/archived) and google-genai (requires pydantic>=2, which
    conflicts with gradio==3.39.0's pydantic<2 requirement). Mimics just
    enough of the old langchain ChatModel interface (callable with a list of
    System/Human/AIMessage, plus get_num_tokens_from_messages) for the rest
    of this codebase to keep working unchanged.
    '''
    ...
    def _split_messages(messages: list):
        ...
        for m in messages:
            if isinstance(m, SystemMessage):
                # merge multiple system messages, just in case
    ...
    def _post(self, endpoint: str, body: dict):
        if not self.api_key:
            raise RuntimeError('GOOGLE_API_KEY is not set. Enter it in the app, or set it as an environment variable.')
        url = f"{self._BASE_URL}/{self.model_name}:{endpoint}?key={self.api_key}"
        resp = requests.post(url, json=body, timeout=120)
        if not resp.ok:
            raise RuntimeError(f"Gemini API error {resp.status_code} for model {self.model_name}: {resp.text[:500]}")
        return resp.json()

    def get_num_tokens_from_messages(self, messages: list):
        system_instruction, contents = self._split_messages(messages)
        body = {'contents': contents}
        if system_instruction:
            body['systemInstruction'] = {'parts': [{'text': system_instruction}]}
        try:
            data = self._post('countTokens', body)
    ...
    def __call__(self, messages: list, max_tokens: int = None, **kwargs):
        system_instruction, contents = self._split_messages(messages)
        ...
        if not candidates:
            ...
            raise RuntimeError(f"Gemini returned no candidates (blockReason={block_reason}): {data}")
...
class Bidder(BaseModel):
    ...
    def _parse_llm(self):
        if 'gemini' or 'gemma' in self.model_name:
            self.llm = GeminiChat(model_name=self.model_name, temperature=self.temperature)
    ...
    def _run_llm_standalone(self, messages: list):
        ...
        for i in range(6):
            try:
                ...
                last_error = e
                print(f'Retrying for {self.model_name} ({i+1}/6) after error: {e!r}, wait for {2**(i+1)} sec...')
                time.sleep(2**(i+1))
        if last_error is not None:
            raise RuntimeError(f"All 6 retries failed calling {self.model_name}: {last_error!r}") from last_error
        # Gemini pricing isn't tracked via a callback like OpenAI's; left at 0.
