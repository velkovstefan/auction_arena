...
BIDDER_NUM = 2
items = create_items('data/example/items_demo.jsonl')
...
def auction_loop_app(*args):
    ...
    items_id = args[1]
    os.environ['GOOGLE_API_KEY'] = args[2] if args[2] != '' else os.environ.get('GOOGLE_API_KEY', '')
    thread_num = args[3]
    item_shuffle = args[4]
    enable_discount = args[5]
    min_markup_pct = args[6]
    args = args[7:]
...
with gr.Blocks(css=customCSS, theme=small_and_beautiful_theme) as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=4):
            with gr.Row():
                google_key = gr.Textbox(label="Google API Key (Gemini)", value="", type="password", placeholder="AIza..")
                    ...
                    with gr.Column(scale=4):
                        with gr.Tab(label=f'Parameters'):
                            model_name = gr.Dropdown(
                                ...
                                info='Model name must contain "gemini" to be recognized. Check ai.google.dev/gemini-api/docs/models for the current list if these become unavailable.',
                            )
                            budget = gr.Number(
                                value=10000, 
                                label='Budget ($)'
                        ...
                        with gr.Tab(label='Monitors'):
                            ...
                            with gr.Row():
                                ...
                                money_monitor = gr.Number(
                                    label='API Cost ($)', 
                                    info='Cost tracking not implemented for Gemini; always 0.',
                                    interactive=False
                                )
    ...
    start_args = dict(
        fn=auction_loop_app,
        inputs=[bidder_list_state, items_checkbox, google_key, thread_num, item_shuffle, enable_discount, min_markup_pct] + bidder_info_gr,
